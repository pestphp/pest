...
it('does not run user hooks when replaying cached skipped and incomplete results', function (): void {
    ...
    $configuration = ['--configuration', 'tests/Fixtures/Suites/TiaReplayHooks.xml'];
    ...
    try {
        ...
        $graph->setFingerprint(Fingerprint::compute($projectRoot, $configuration));
        ...
        $process = new Process(
            ['php', 'bin/pest', ...$configuration, '--tia'],
            $projectRoot,
            [
