...
it('throws exception if dataset does not exist', function () {
    ...
    DatasetsRepository::resolve('foo', [new TestDataset('first', [])]);
});
...
it('sets closures', function () {
    ...
    expect(DatasetsRepository::resolve('foo', [new TestDataset('foo', [])]))->toBe(['foo with (1)' => [1]]);
});

it('sets arrays', function () {
    DatasetsRepository::setGlobalDataset('bar', [[2]]);

    expect(DatasetsRepository::resolve('bar', [new TestDataset('bar', [])]))->toBe(['bar with (2)' => [2]]);
});
...
it('supports dataset parameters', function (string $name) use ($state) {
    $state->iterations++;
...
it('supports dataset optional parameters', function ($name) use ($state) {
    $state->iterations++;

    expect($name)->toBe(strtolower($name));
})->with('with_arguments');

test('dataset optional parameters did the job right', function () use ($state) {
    expect($state->iterations)->toBe(5);
});

it('supports dataset named parameters', function ($name) use ($state) {
    $state->iterations++;

    expect($name)->toBe(strtoupper($name));
})->with('with_arguments', uppercase: true);
