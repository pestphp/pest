...
describe('getShard', function () {
    ...
    it('throws exception for invalid format', function (array $arguments, string $errorPattern) {
        $input = new ArgvInput($arguments);

        Shard::getShard($input);
    })->with([
        [['test', '--shard', 'invalid'], 'must be in the format "index/total"'],
        [['test', '--shard', '1'], 'must be in the format "index/total"'],
        [['test', '--shard', '1/'], 'must be in the format "index/total"'],
        [['test', '--shard', '/2'], 'must be in the format "index/total"'],
        [['test', '--shard', 'a/b'], 'must be in the format "index/total"'],
        [['test', '--shard', '1.5/2'], 'must be in the format "index/total"'],
    ])->throws(InvalidOption::class);

    it('throws exception for invalid index or total values', function (array $arguments, string $errorPattern) {
        $input = new ArgvInput($arguments);

        Shard::getShard($input);
    })->with([
        [['test', '--shard', '0/2'], 'must be a non-negative integer'],
        [['test', '--shard', '1/0'], 'must be a non-negative integer'],
        [['test', '--shard', '-1/2'], 'must be a non-negative integer'],
        [['test', '--shard', '1/-2'], 'must be a non-negative integer'],
        [['test', '--shard', '3/2'], 'must be a non-negative integer less than the total'],
        [['test', '--shard', '5/4'], 'must be a non-negative integer less than the total'],
    ])->throws(InvalidOption::class);
});
...
describe('buildFilterArgument', function () {
    ...
    it('generates compact filter for multiple tests with common prefix', function () {
        ...
        // Should create a compact regex using tree structure
    ...
    it('generates compact filter for tests with different namespaces', function () {
        ...
        // Should create a compact regex with branches for Unit and Feature
...
describe('ensureFilterLengthIsSafe', function () {
    ...
    it('throws exception when filter exceeds default limit', function () {
        ...
        // Create a filter that exceeds 32KB
...
describe('handleArguments', function () {
    ...
    it('generates shard filter when shard option is present', function () {
        $output = new BufferedOutput;
        $shard = new Shard($output);

        $arguments = ['bin/pest', 'tests/', '--parallel', '--shard=1/2'];
...
describe('addOutput', function () {
    ...
    it('returns original exit code when shard is not set', function () {
        ...
        // Reset static shard property to ensure it's null
        $reflection = new ReflectionClass($shard);
        $property = $reflection->getProperty('shard');
